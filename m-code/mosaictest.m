...
% Extracting subdiagonal to get average distance, 1.8 is arbitrary
neighborlimit = mean(diag(d,1)) * 1.8;

% Narrow arrays to the straight parts
pos = pos(str&up,:);
imageIDs = imageIDs(str&up,1);
d = d(str&up,str&up);
...
% Scatter plot of the straight drone positions in XY plane for a random image
...
for ids=1:size(imageIDs)
    ...
    composite = ipaste(composite, im, pos(ids,1:2) * (1/cf) + [15000 1600], 'set');
    imshow(composite)
end
cd ..
imshow(composite)
...
% s0=+ H(7) ;
        ...
        %         H = H / H(3,3) ;
        %         opts = optimset('Display', 'none', 'TolFun', 1e-8, 'TolX', 1e-8) ;
        %         H(1:8) = fminsearch(@residual, H(1:8prob2dbatstruct(prob);
% ss0=s0;
%
