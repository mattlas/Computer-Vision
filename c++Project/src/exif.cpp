...
public:
	...
	EXIFEntry(const EXIFEntry &) = delete;
	EXIFEntry &operator=(const EXIFEntry &) = delete;
	EXIFEntry(EXIFEntry &&other) :
			tag_(other.tag_), format_(other.format_), data_(other.data_), length_(
					other.length_), val_byte_(other.val_byte_){
		other.tag_ = 0xFF;
		other.format_ = 0xFF;
		other.data_ = 0;
		other.length_ = 0;
...
bool extract_values(C &container, const unsigned char *buf, const unsigned base,
		const unsigned len, const EXIFEntry &entry){
	...
	if(sizeof(T) * entry.length() <= 4){
		if(alignIntel){
			...
			reversed_data = entry.data();
			unsigned char *data =
					reinterpret_cast<unsigned char *>(&reversed_data);
			unsigned char tmp;
			tmp = data[0];
			data[0] = data[3];
			data[3] = tmp;
...
void parseEXIFEntryHeader(const unsigned char *buf, unsigned short &tag,
...
template<bool alignIntel>
void parseEXIFEntryHeader(const unsigned char *buf, EXIFEntry &result){
	unsigned short tag;
	unsigned short format;
	unsigned length;
	unsigned data;

	parseEXIFEntryHeader<alignIntel>(buf, tag, format, length, data);
...
template<bool alignIntel>
EXIFEntry parseEXIFEntry_temp(const unsigned char *buf, const unsigned offs,
	...
	parseEXIFEntryHeader<alignIntel>(buf + offs, result);
...
void parseEXIFEntryHeader(const unsigned char *buf, bool alignIntel,
		unsigned short &tag, unsigned short &format, unsigned &length,
		unsigned &data){
	if(alignIntel){
		parseEXIFEntryHeader<true>(buf, tag, format, length, data);
	}else{
		parseEXIFEntryHeader<false>(buf, tag, format, length, data);
	}
}

EXIFEntry parseEXIFEntry(const unsigned char *buf, const unsigned offs,
		const bool alignIntel, const unsigned base, const unsigned len){
	if(alignIntel){
		return parseEXIFEntry_temp<true>(buf, offs, base, len);
	}else{
		return parseEXIFEntry_temp<false>(buf, offs, base, len);
	}
}
}

int exif::EXIFInfo::parseFrom(const unsigned char *buf, unsigned len){
	if(!buf || len < 4)
...
int exif::EXIFInfo::parseFrom(const string &data){
	return parseFrom(reinterpret_cast<const unsigned char *>(data.data()),
			static_cast<unsigned>(data.length()));
}

int exif::EXIFInfo::parseFromEXIFSegment(const unsigned char *buf,
		unsigned len){
	...
	while(--num_entries >= 0){
		EXIFEntry result = parseEXIFEntry(buf, offs, alignIntel,
	...
	if(exif_sub_ifd_offset + 4 <= len){
		...
		while(--num_entries >= 0){
			...
			case 0xa20e:
				if(result.format() == 5){
					this->LensInfo.FocalPlaneXResolution =
							result.val_rational()[0];
				}
				break;

			case 0xa20f:
				if(result.format() == 5){
					this->LensInfo.FocalPlaneYResolution =
							result.val_rational()[0];
				}
				break;
			...
			case 0xa432:
				if(result.format() == 5){
					...
					if(sz)
						this->LensInfo.FocalLengthMin =
								result.val_rational()[0];
					if(sz > 1)
						this->LensInfo.FocalLengthMax =
								result.val_rational()[1];
					if(sz > 2)
	...
	if(gps_sub_ifd_offset + 4 <= len){
		...
		while(--num_entries >= 0){
			...
			parseEXIFEntryHeader(buf + offs, alignIntel, tag, format, length,
					data);
			switch(tag){
			case 1:
			...
			case 2:
				if((format == 5 || format == 10) && length == 3){
					this->GeoLocation.LatComponents.degrees = parse_value<
							Rational>(buf + data + tiff_header_start,
							alignIntel);
					this->GeoLocation.LatComponents.minutes = parse_value<
							Rational>(buf + data + tiff_header_start + 8,
							alignIntel);
					this->GeoLocation.LatComponents.seconds = parse_value<
							Rational>(buf + data + tiff_header_start + 16,
							alignIntel);
			...
			case 4:
				if((format == 5 || format == 10) && length == 3){
					this->GeoLocation.LonComponents.degrees = parse_value<
							Rational>(buf + data + tiff_header_start,
							alignIntel);
					this->GeoLocation.LonComponents.minutes = parse_value<
							Rational>(buf + data + tiff_header_start + 8,
							alignIntel);
					this->GeoLocation.LonComponents.seconds = parse_value<
							Rational>(buf + data + tiff_header_start + 16,
							alignIntel);
